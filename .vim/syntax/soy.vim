...
syn region soyComment start=/\m\/\// end=/\m$/ contains=soyCommentLinks
syn region soyComment start=/\m\/\*/ end=/\m\*\// contains=soyDocumentationParam,soyCommentLinks
" soyDocumentation*
" /**
...
syn match soyExprVar contained /\m$\w\+[[:alnum:]\[\]?.$]*/
...
syn keyword soyExprFunc contained isFirst isLast index length hasData keys round floor ceiling min max randomInt nextgroup=soyFuncInner skipwhite
...
syn region soyNamespaceCommand matchgroup=soyBraces start=/\m{namespace/ end=/\m}/ contains=soyNamespaceArg,soyExprString
...
syn region soyMsgCommand matchgroup=soyBraces start=/\m{msg/ end=/\m}/ contains=soyMsgArg,soyExprString
...
syn region soyI18nCommand matchgroup=soyBraces start=/\m{\(plural\|select\)/ end=/\m}/ contains=soyExpr.*
" soyOtherSimpleCommands: Matches any commands that does not contain any
"     parameters.
syn match soyOtherSimpleCommands /\m{\(literal\|default\)}/
...
SoyHiLink soyOtherSimpleCommands Statement
